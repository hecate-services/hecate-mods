...
%%% `note_participant_left' (repeatable, lifecycle facts only, no
%%% content) -> `end_room_moderation' (death, terminal). Status/lifecycle
%%% GATING lives here, in `execute/2', not in the desk handlers -- same
%%% split as `guide_mailbox_lifecycle''s `mailbox_aggregate'.
%%% @end
