...
%% data_dir/0 from hecate_mods_service. hecate_om:boot/1 picks them up and starts
%% the store plus its evoq subscription BEFORE start/1 fires, so you never call
%% reckon_db_sup:start_store/1 yourself.
-module(hecate_mods_app).

-behaviour(application).

-export([start/2, stop/1]).

start(_Type, _Args) -> hecate_om:boot(hecate_mods_service).
